...
1. **データ取得** (PR-S1): Source Protocol 経由で OHLCV / 銘柄メタ / 営業日 / 主要指数を `data/cache/` に保存。`data_snapshot_hash` を生成。
2. **decision_trace 生成** (PR-S2 / S3): bar_ts ごとに trace JSONL を `runs/<run_id>/trace_raw.jsonl` に append。
3. **future_outcome enrich** (PR-S3): bar_ts + N 日経過後に forward_return / mfe / mae / outcome_label を `runs/<run_id>/outcome_backfill.jsonl` に append。
