...
| PR-S4 | trace-stats MVP (`kabu.stats`、aggregate / cross / report、bucket 境界 docs 化) | **完了** (commit 20928b5、CI green 確認済) |
| P4.5 | attribution MVP (`kabu.attribution`: symbol / sector / period / rule_id / skip_reason / outcome、concentration warning、observation-only phrasing) | **完了** (commit 9b947ef、CI green 確認済) |
| P4.7 | vendor adapter 最小確認 (`kabu.data.sources.yfinance_source`、lazy import / optional extra / 構造 isolation) | **本 PR で完了予定** |
| PR-S5 | market_index_context + sector_context (PR-S7 後の予定だが順序再確認) | 未着手 (要承認) |
...
| 9 | PR-S4 | trace-stats MVP (`kabu.stats`: loaders / buckets / aggregate / cross / report) | **完了** (commit 20928b5) | TRACE_ANALYSIS_WORKFLOW.md / STATS.md | test_load_run_inputs_requires_metadata / test_load_run_inputs_run_id_mismatch / test_stats_header_contains_bias_warnings / test_aggregate_final_action_outcome / test_aggregate_symbol_outcome / test_skip_reason_counts / test_trade_pnl_summary / test_low_sample_bucket_flag / test_bucket_boundaries_documented / test_cross_stats_two_axis / test_stats_writer_uses_tmp_path |
| 10 | P4.5 | attribution MVP (`kabu.attribution`: aggregate axes / concentration warnings / report writer) | **完了** (commit 9b947ef) | ATTRIBUTION.md / STATS.md / TRACE_ANALYSIS_WORKFLOW.md | test_attribution_symbol_pnl / test_attribution_period_year_month / test_attribution_sector_unknown_when_missing / test_attribution_rule_id / test_attribution_skip_reason / test_concentration_warning_for_single_symbol / test_contribution_pct_handles_zero_total / test_attribution_header_contains_bias_warnings / test_attribution_writer_uses_tmp_path / test_attribution_is_observation_not_recommendation |
| 11 | P4.7 | vendor adapter (`kabu.data.sources.yfinance_source`): lazy import / optional extra / mock-only CI / 構造 isolation | **完了予定 (本 PR)** | DATA_SOURCES.md / VENDOR_SETUP.md | test_vendor_source_requires_as_of / test_vendor_source_filters_by_as_of / test_vendor_parser_to_ohlcbar / test_no_real_yfinance_imported / test_vendor_layer_isolation / test_no_raw_data_committed |
